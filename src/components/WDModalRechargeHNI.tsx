...
interface Props {
  ...
  max?: BigNumber
  min?: BigNumber
  action: (amount: bigint) => Promise<Res>
}
...
export default function WithdrawModal({
  ...
  const handleAction = async () => {
    ...
    try {
      const res: Res = await action(amountToken.toBigInt as bigint)
        ...
        <DialogContent>
          ...
          {amountError && (
            <FormHelperText error>
              Please provide valid UBQ amount (max 16 decimals). Error: {amountError.message}
            </FormHelperText>
          )}
        </DialogContent>
        <DialogActions>
