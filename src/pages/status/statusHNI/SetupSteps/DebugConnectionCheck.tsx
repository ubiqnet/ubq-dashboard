...
export default function NodeConnectionCheck({ isOk }: Props): ReactElement | null {
  const { setHniDebugApiUrl, apiHniDebugUrl } = useContext(SettingsContext)

  const changeDebugApiUrl = (
    <div style={{ display: 'flex', marginTop: '25px', marginBottom: '25px' }}>
      <span style={{ marginRight: '15px' }}>
        Debug API (<Typography variant="button">{apiHniDebugUrl}</Typography>)
      </span>
      <ConnectToHost
        setHost={(host: string) => {
          console.log(host) // eslint-disable-line
          setHniDebugApiUrl(host)
        }}
        defaultHost={apiHniDebugUrl}
      />
    </div>
  )
    ...
    <div>
      ...
      <div>
        <Typography component="div" variant="body2" gutterBottom style={{ margin: '15px' }}>
          We cannot connect to your nodes UBQ debug API at <Typography variant="button">{apiHniDebugUrl}</Typography>.
          Please check the following to troubleshoot your issue.
            ...
            <AccordionDetails>
              <Typography component="div">
                <ol>
                  ...
                  <li>
                    If your node is running, check your firewall settings to make sure that port 8080 (or your custom
                  ...
                  <MuiAlert
                    ...
                    severity="error"
                  >
                    Your UBQ debug node API should never be completely open to the internet. If you want to connect
                  ...
                  />
                  <li>
                    Lastly, check your nodes configuration settings to validate the UBQ debug API is enabled and the
                    Cross Origin Resource Sharing (CORS) setting is configured to allow your host. Config parameter{' '}
                    <strong>UBQ-debug-api-enable</strong> must be set to <strong>true</strong> and{' '}
